...
@Scope("prototype")
@Slf4j
public class CustomExcelListener extends AnalysisEventListener<ExcelCustom> {
    ...
    //缓存
    List<ExcelCustom> linkUserList = new ArrayList<>();

    int currentRow;

    Class clazz= Class.forName("com.jxm.business.dto.ExcelCustom");

    public CustomExcelListener() throws ClassNotFoundException {
        super();
    }

    @Override
    public void invoke(ExcelCustom data, AnalysisContext context) {
        if(data.getCustomName()!=null)
        linkUserList.add(data);
        if (linkUserList.size() % SIZE == 0) {
            exportExcelHandleService.saveCustomInfo(linkUserList);
            linkUserList.clear();//清空
        }
    }
