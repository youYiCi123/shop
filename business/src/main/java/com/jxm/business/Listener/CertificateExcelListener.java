...
@Scope("prototype")
@Slf4j
public class CertificateExcelListener extends AnalysisEventListener<ExcelCertificate> {
    ...
    //缓存
    List<ExcelCertificate> certificateList = new ArrayList<>();

    int currentRow;

    Class clazz= Class.forName("com.jxm.business.dto.ExcelCertificate");

    public CertificateExcelListener() throws ClassNotFoundException {
        super();
    }

    @Override
    public void invoke(ExcelCertificate data, AnalysisContext context) {
        if(data.getCertificateName()!=null)
            certificateList.add(data);
        if (certificateList.size() % SIZE == 0) {
            exportExcelHandleService.saveCertificateInfo(certificateList);
            certificateList.clear();//清空
        }
    }
